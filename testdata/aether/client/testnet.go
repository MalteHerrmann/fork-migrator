...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		coins := sdk.Coins{
			sdk.NewCoin(aethertypes.Attoaether, accStakingTokens),
		}
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(aethertypes.Attoaether, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(aethertypes.Attoaether)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, aethertypes.Attoaether, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
